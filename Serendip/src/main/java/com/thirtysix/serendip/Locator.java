...
import com.thirtysix.serendip.model.GeoLocation;
...
public class Locator {
	...
	public GeoLocation getCurrentGeoLocation() {
		final LocationManager locationService = (LocationManager) context
				.getSystemService(Context.LOCATION_SERVICE);
		return askAllProviders(context, locationService,
				locationService.getAllProviders());
	}

	private GeoLocation askAllProviders(Context context,
			final LocationManager locationService,
			final List<String> allProviders) {
		for (String provider : allProviders) {

			final GeoLocation lastKnownGeoLocation = askProvider(context,
	...
	private GeoLocation askProvider(Context context,
			final LocationManager locationService, String provider) {
		// Log.d("way", String.format("---provider:%s", provider));

		final Location lastKnownLocation = locationService
				.getLastKnownLocation(provider);
		if (lastKnownLocation == null)
			return null;
		final List<Address> fromLocation = myGeoCoder.getFromLocation(context,
	...
	private String makeAddress(Address address) {
		return String.format("%s,%s", address.getAddressLine(0),
